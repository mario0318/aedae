function Invoke-AeDaeProjectSdkCheck {
    ...
    foreach ($project in $ProjectFiles) {
        $matches = [regex]::Matches((Get-Content $project -Raw), '<WindowsTargetPlatformVersion>(?<v>[^<]+)</WindowsTargetPlatformVersion>')
        if ($matches.Count -ne 1 -or $matches[0].Groups['v'].Value.Trim() -ne $manifest.sdkVersion) { throw "Pinned build project targets an unapproved Windows SDK: $project" }
    }
}
function Invoke-AeDaeHeaderCheck {
    param([string]$IncludeRoot, [string]$ManifestPath)
    $manifest = Get-Content $ManifestPath -Raw | ConvertFrom-Json
    foreach ($header in $manifest.headers.PSObject.Properties) { $actual=(Get-FileHash (Join-Path $IncludeRoot $header.Name) -Algorithm SHA256).Hash; if($actual -ne $header.Value){throw "$($header.Name) hash mismatch. Expected $($header.Value), got $actual."} }
